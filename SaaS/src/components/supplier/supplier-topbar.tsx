...
export function SupplierTopBar({ user }: SupplierTopBarProps) {
    const [isSwitchingRole, setIsSwitchingRole] = useState(false);

    // 開発環境用のロール切り替え権限チェック（他ロールと同様に緩和済み）
    const canRoleSwitch =
        !!user &&
        process.env.NODE_ENV !== 'production';

    // MVP用: ロールに応じた表示名
    const displayName = user?.role === 'SUPPLIER' ? '佐世保食品' : user?.name || user?.email || '';
